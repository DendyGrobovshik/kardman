...
1. **Kernel compiler plugin** — generates C++ glue + `rdma_classes.json` + injects vtable
...
The FIR compiler plugin (`:rdma-compiler-plugin`) is applied to the plugin module's JVM resolve compilation via the Gradle wrapper `:rdma-gradle-plugin`. It receives two options, defaulting to:

```kotlin
// rdma-gradle-plugin/src/main/kotlin/.../RdmaIrGradlePlugin.kt
SubpluginOption("rdmaClassesJson", "${rootProject.projectDir}/kernel/build/generated/rdma/rdma_classes.json")
...
Note: `:rdma-compiler-plugin` and `:rdma-gradle-plugin` are published to `mavenLocal()`. After changing them, run:

```bash
./gradlew :rdma-compiler-plugin:publishToMavenLocal :rdma-gradle-plugin:publishToMavenLocal
```
...
./gradlew :kernel:compileKotlinJvm
cat kernel/build/generated/rdma/rdma_classes.json
