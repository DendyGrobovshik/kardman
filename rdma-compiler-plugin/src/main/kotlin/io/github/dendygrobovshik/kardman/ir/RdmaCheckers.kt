...
class RdmaAdditionalCheckersExtension(session: FirSession) : FirAdditionalCheckersExtension(session) {

    override val declarationCheckers: DeclarationCheckers = object : DeclarationCheckers() {
        override val fileCheckers = setOf(RdmaFileChecker)
        override val regularClassCheckers = setOf(RdmaRegularClassChecker)
    }

    override val expressionCheckers: ExpressionCheckers = object : ExpressionCheckers() {
        override val functionCallCheckers = setOf(RdmaFunctionCallChecker)
        override val propertyAccessExpressionCheckers = setOf(RdmaPropertyAccessChecker)
        override val variableAssignmentCheckers = setOf(RdmaVariableAssignmentChecker)
    }
}
...
object RdmaFileChecker : FirDeclarationChecker<FirFile>(MppCheckerKind.Common) {
    ...
    override fun check(declaration: FirFile) {
        ...
        RdmaTransformState.registerText(path, text)

        for (imp in declaration.imports) {
            val fqn = imp.importedFqName?.asString() ?: continue
            if (!RdmaTransformState.isRdmaQualifiedName(fqn)) continue
            val src = imp.source ?: continue
            RdmaTransformState.addEdit(path, src.startOffset, endWithNewline(text, src.endOffset), "")
        }
    }
}

object RdmaRegularClassChecker : FirDeclarationChecker<FirRegularClass>(MppCheckerKind.Common) {
    context(context: CheckerContext, reporter: DiagnosticReporter)
    override fun check(declaration: FirRegularClass) {
        val superRef = declaration.superTypeRefs.firstOrNull { ref ->
            ...
            fqn != null && RdmaTransformState.isRdmaQualifiedName(fqn)
        } ?: return
        ...
        val text = RdmaTransformState.textOf(path)
        ...
        RdmaTransformState.registerSubclass(
            qualifiedName,
...
object RdmaFunctionCallChecker : FirExpressionChecker<FirFunctionCall>(MppCheckerKind.Common) {
    context(context: CheckerContext, reporter: DiagnosticReporter)
    override fun check(expression: FirFunctionCall) {
        ...
        val subclass = RdmaTransformState.subclassByQualifiedName(fqn)
        if (subclass != null) {
            ...
            RdmaTransformState.addEdit(path, src.startOffset, src.endOffset, replacement)
            return
        }

        val type = RdmaTransformState.typeByQualifiedName(fqn) ?: return
...
object RdmaPropertyAccessChecker : FirExpressionChecker<FirPropertyAccessExpression>(MppCheckerKind.Common) {
    context(context: CheckerContext, reporter: DiagnosticReporter)
    override fun check(expression: FirPropertyAccessExpression) {
        ...
        RdmaTransformState.addEdit(path, accessSrc.startOffset, accessSrc.endOffset, "$receiver$getter()")
    }
}

object RdmaVariableAssignmentChecker : FirExpressionChecker<FirVariableAssignment>(MppCheckerKind.Common) {
    context(context: CheckerContext, reporter: DiagnosticReporter)
    override fun check(expression: FirVariableAssignment) {
        ...
        RdmaTransformState.addEdit(path, src.startOffset, src.endOffset, "$receiver$setter($rValueText)")
    }
}
