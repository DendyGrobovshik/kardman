...
object RdmaTransformState {
    var outputDir: String = ""
    var types: List<RdmaIrType> = emptyList()
        private set
    ...
    fun configure(jsonPath: String?, outputDir: String?) {
        ...
        val file = File(path)
        if (file.exists()) {
            types = RdmaConfig.parseClassesJson(file.readText())
        }
    }

    fun typeByQualifiedName(fqn: String): RdmaIrType? = types.find { it.qualifiedName == fqn }
